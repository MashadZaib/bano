...
const forgetPassword = async (req, res) => {
  ...
  if (!isAdded) {
    return res.status(404).send({
      message: "User/User Not found with this email!",
    });
  } else {
  ...
  try {
    ...
    if (isAdded) {
      ...
      const newUser = new User({
        ...
        joiningDate: req.body.userData.joiningDate,
...
const updateUser = async (req, res) => {
  try {
    ...
    // console.log('Updating user from Master DB', admin);
    if (admin) {
